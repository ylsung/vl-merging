...
class ImageNetDataset(TSVCompositeDataset):
    def __init__(self, data_dir, *args, split="", **kwargs):
        if split == "train":
            yaml_file = op.join(data_dir, "train_imagenet-22k_30.yaml")
        elif split == "val":
            yaml_file = op.join(data_dir, "val_imagenet-22k.yaml")
        else:
            yaml_file = op.join(data_dir, "val_imagenet-22k.yaml")
...
if __name__ == "__main__":
    dset = ImageNetDataset(
        ...
        data_dir="/storage/v-yilinsung/imagenet-22k/composite/",
