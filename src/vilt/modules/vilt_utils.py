    ...
    for split in ["train", "val"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                # setattr(pl_module, f"{split}_{k}_vl_loss", Scalar())
...
def epoch_wrapup(pl_module):
    ...
    if phase == "train":
        try:
            pl_module.print_trainable_params_percentage()
        except:
            print("There are issues when printing params percentage")
    ...
    if pl_module.hparams.config["get_recall_metric"] and not pl_module.training:
        ...
        print((val_avg, test_avg), pl_module.global_step)
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            ...
            value = getattr(pl_module, f"{phase}_{loss_name}_vl_i2t_accuracy").compute()
            pl_module.log(f"{loss_name}/{phase}/vl_i2t_accuracy_epoch", value)
            getattr(pl_module, f"{phase}_{loss_name}_vl_i2t_accuracy").reset()
            
            value = getattr(pl_module, f"{phase}_{loss_name}_vl_t2i_accuracy").compute()
            pl_module.log(f"{loss_name}/{phase}/vl_t2i_accuracy_epoch", value)
...
def set_schedule(pl_module):
    ...
    beta_2 = pl_module.hparams.config["beta_2"]
    ...
    head_names = ["vqa_classifier", "nlvr2_classifier", "img_cls_classifier"]

    if pl_module.hparams.config["all_mlp_mult"]:
        head_names.append("mlp")

    if pl_module.hparams.config["all_vl_mult"]:
        head_names.append("attn.vl")
        head_names.append("mlp.vl")
        head_names.append("mlp_vl")

    if pl_module.hparams.config["all_v_mult"]:
        head_names.append("attn.v")
        head_names.append("mlp.v")

    if pl_module.hparams.config["all_l_mult"]:
        head_names.append("attn.l")
        head_names.append("mlp.l")
    ...
    optimizer_grouped_parameters = [
        ...
        },
        {
            ...
            "weight_decay": pl_module.hparams.config["weight_decay_custom_modules"],
            "lr": lr * lr_mult,
        },
        {
            "params": [
    ...
    if optim_type == "adamw":
        optimizer = AdamW(
            optimizer_grouped_parameters, lr=lr, eps=1e-8, betas=(0.9, beta_2)
        )
    ...
    if pl_module.trainer.max_steps is None or pl_module.trainer.max_steps == -1: # different version of pytorch lightning uses different default value
        max_steps = (
